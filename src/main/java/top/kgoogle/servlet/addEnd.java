...
@WebServlet(name = "addEnd",urlPatterns = "/check/addEnd")
public class addEnd extends HttpServlet {
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if(ud.getHibernateTemplate().findByCriteria(detachedCriteria).size()>0) {
            ...
            c.end=request.getParameter("nEnd");
            ud.update(c);
            response.setContentType("text/html; charset=utf-8");
            Writer writer = response.getWriter();
            writer.write("更新成功~,点击<a href=\"/check/norManage.jsp\">这里</a>进行跳转");
            writer.close();
        }else{
            response.setContentType("text/html; charset=utf-8");
            Writer writer = response.getWriter();
